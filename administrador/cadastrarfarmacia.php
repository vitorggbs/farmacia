...
$categoriasIniciais = array('Medicamentos', 'Higiene', 'Cosméticos', 'Infantil', 'Suplementos', 'Outros');
$stmtCategoria = mysqli_prepare($conexao, 'INSERT INTO categorias (farmacia_id, nome) VALUES (?, ?)');
foreach ($categoriasIniciais as $categoriaNome) {
    mysqli_stmt_bind_param($stmtCategoria, 'is', $farmaciaId, $categoriaNome);
    if (!mysqli_stmt_execute($stmtCategoria)) {
        mysqli_rollback($conexao);
        header('Location: farmacias.php?erro=banco#cadastrar');
        exit;
    }
