...
function cabecalhoAdmin($titulo)
{
    ...
    echo '<form method="POST" action="'.htmlspecialchars($raiz, ENT_QUOTES, 'UTF-8').'logout.php" class="d-none d-lg-inline-flex">'.campoCsrf().'<button class="btn btn-sair rounded-pill px-3">SAIR</button></form>';
    ...
    echo '</nav>';
    echo '<form method="POST" action="'.htmlspecialchars($raiz, ENT_QUOTES, 'UTF-8').'logout.php" class="mt-auto">'.campoCsrf().'<button class="btn btn-primary rounded-pill w-100">SAIR</button></form>';
    echo '</div>';
    echo '</div>';
}
