...
$stmtCategorias = mysqli_prepare($conexao, 'SELECT id, nome FROM categorias WHERE ativo = 1 AND farmacia_id = ? ORDER BY nome');
mysqli_stmt_bind_param($stmtCategorias, 'i', $farmaciaId);
mysqli_stmt_execute($stmtCategorias);
$categorias = mysqli_stmt_get_result($stmtCategorias);
