...
function recursosCabeca(string $titulo): void
{
    ...
    <meta name="csrf-token" content="<?php echo htmlspecialchars(tokenCsrf(), ENT_QUOTES, 'UTF-8'); ?>">
...
function renderizarAgenteIa(): void
{
    ...
    <button id="farmaAiLauncher" class="farma-ai-launcher" type="button"
            aria-label="Abrir assistente FarmaCerta" aria-controls="farmaAiPanel" aria-expanded="false">
        <span class="farma-ai-dot" aria-hidden="true"></span>
        <img src="<?php echo $prefixo; ?>assets/logo_farmacia.png" alt="Logo FarmaCerta" class="farma-ai-icon-img" width="32" height="32">
    </button>

    <section id="farmaAiPanel" class="farma-ai-panel" aria-hidden="true" aria-label="Chat com o assistente FarmaCerta">
        <header class="farma-ai-header">
            <div class="farma-ai-avatar" aria-hidden="true">
                <img src="<?php echo $prefixo; ?>assets/logo_farmacia.png" alt="Logo FarmaCerta" width="24" height="24">
            </div>
            <div class="farma-ai-heading">
                <strong>FarmaCerta IA</strong>
                <span><?php echo $identidade; ?> · Google Cloud</span>
            </div>
            <button id="farmaAiClose" class="farma-ai-close" type="button" aria-label="Fechar assistente">×</button>
        </header>
...
function recursosRodape(): void
{
    ...
    $arquivoScriptSelect = __DIR__ . '/../assets/js/custom-select.js';
    $versaoScriptSelect = is_file($arquivoScriptSelect) ? filemtime($arquivoScriptSelect) : time();
    ...
    echo '<script src="' . $prefixoRaw . 'assets/js/custom-select.js?v=' . $versaoScriptSelect . '"></script>';
    echo '<script>';
    echo 'document.querySelectorAll("form[method=POST],form[method=post]").forEach(function(form){if(!form.querySelector("input[name=_csrf]")){var input=document.createElement("input");input.type="hidden";input.name="_csrf";input.value=(document.querySelector("meta[name=csrf-token]")||{}).content||"";form.appendChild(input);}});';
